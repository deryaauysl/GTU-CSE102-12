...
CustomDict *create_dict()
{
    ...
    dict->items = NULL;
    dict->capacity = 0; // SOR capacity ve size farkı nedir
    dict->size = 0;
    return dict;
}
...
void add_item(CustomDict *dict, char *key, Value *value, char *dtype)
{
    ...
    {
        dict->capacity = (dict->capacity == 0) ? 1 : dict->capacity * 2; // SOR
    ...
    for (i = 0; i < dict->size; i++)
    {
        if (strcmp(dict->items[i].key, key) == 0)
        {
            ...
            *(dict->items[i].value) = *value;
            ...
            dict->items[i].dtype = strdup(dtype); // SOR strdup ne işe yarar
            return;
        }
    }
...
void print_dict(CustomDict *dict)
{
    ...
    for (i = 0; i < dict->size; i++)
    {
        ...
        {
            printf("%d ", dict->items[i].value->f);
        }
        else if (strcmp(dict->items[i].dtype, "double") == 0)
        {
            printf("%d ", dict->items[i].value->d);
        }
        else if (strcmp(dict->items[i].dtype, "char") == 0)
        {
            printf("%d ", dict->items[i].value->c);
        }
        printf("\n");
    }
}
...
int read_csv(CustomDict *dict, const char *filename)
{
    ...
    {
        char *type = strtok(line, ","); // SOR strtok
    ...
    fclose(file);
    return 1; // SOR bu kodları terminalde nasıl çalıştırabilirim?
}